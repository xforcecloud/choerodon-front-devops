...
class SelectAppStore {
  ...
  loadOrganizationApps = (
    ...
    this.changeLoading(true);
    return axios.post(`devops/v1/organizations/${organizationId}/apps/list_by_options?page=${page}&size=${size}&sort=${sort.field},${sort.order}&has_version=true`, JSON.stringify(postData))
      .then((data) => {
        const res = this.handleProptError(data);
        if (res) {
          if (this.searchValue === '' || this.searchValue === postData.param) {
